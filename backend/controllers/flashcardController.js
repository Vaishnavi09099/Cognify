...
export const getflashcards = async (req, res, next) => {
    try {
        ...
        res.status(500).json({ success: false, message: "error fetching flashcard" })
    }
};
...
export const toggleStarFlashcard = async (req, res, next) => {
    try {
        ...
        return res.status(500).json({ success: false, message: "Error is there" })
    }
};
...
export const reviewFlashcard = async (req, res, next) => {
    try {
        ...
        res.status(500).json({ success: false, message: "flashcard not reviewed successfully" })
    }
};
...
export const deleteFlashcardSet = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ success: true, message: "FlashcardSet deleted successfully" })

    } catch (err) {
        res.status(500).json({ success: false, message: "error deleting flashcards" })
    }
}